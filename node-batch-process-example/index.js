...
function process(id) {
  const updateQuery = mysql.format(
    "UPDATE testTable SET is_processed = 1, evenNum= id * 2 WHERE id = ?",
    [id]
  );
  return function (cb) {
...
function batch(batchSize, parallelLimit) {
  const selectQuery = mysql.format(
    "SELECT * FROM testTable WHERE is_processed=0 limit ?",
    [batchSize]
  );

  const statusQuery =
    "SELECT COUNT(*) AS count FROM testTable WHERE is_processed=1";
...
function run(batchSize, parallelLimit, total) {
  ...
  if (total) {
    ...
    dbPool.query(
      "SELECT COUNT(*) AS count FROM testTable where is_processed=0",
      function (err, result) {
        if (err) {
